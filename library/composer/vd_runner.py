...
class RunnerVD:
    ...
    def _get_commands(
        ...
        headless_command: str = "" if ConfigVD.Browser.is_headless() else "--headed"
        browser_command: str = ""
        if ConfigVD.Browser.is_multi_browser():
            for browser_name in ConfigVD.Browser.get_multi_browser_list():
                browser_command += f"--browser {browser_name} "
        else:
